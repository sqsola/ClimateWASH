...
library(data.table)
library(purrr)
library(ethiopianDate, lib = "~/rlibs/R")
library(janitor)
library(tidyverse)
...
for(entry in 1:nrow(countries)) {
  ...
  if (exists("spatial")) {
    ...
    if (name_year == "NG_90") {
      ...
      weather_final <- readRDS(paste0(name_year, "_weatherfinal_cm.rds"))
    }
    ...
    # weather_final <- setDT(weather_final)
    ...
    for (weather_var_accum in accum_var) {
      weather_final <- weather_final %>%
        ...
        mutate(!!paste0(weather_var_accum,"_totalminus15_105"):=(sum(across(num_range(paste0(weather_var_accum,"_"), 15:105))))) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus30_105"):=(sum(across(num_range(paste0(weather_var_accum,"_"), 30:105))))) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus60_105"):=(sum(across(num_range(paste0(weather_var_accum,"_"), 60:105))))) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus1_105"):=(sum(across(num_range(paste0(weather_var_accum,"_"), 1:105))))) %>%
        ...
        mutate(!!paste0(weather_var_accum,"_totalminus15_105_90"):=.data[[!!paste0(weather_var_accum,"_totalminus15_105")]] * 0.9) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus15_105_95"):=.data[[!!paste0(weather_var_accum,"_totalminus15_105")]] * 0.95) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus15_105_99"):=.data[[!!paste0(weather_var_accum,"_totalminus15_105")]] * 0.99) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus15_105_999"):=.data[[!!paste0(weather_var_accum,"_totalminus15_105")]] * 0.999) %>% 
        
        mutate(!!paste0(weather_var_accum,"_totalminus30_105_90"):=.data[[!!paste0(weather_var_accum,"_totalminus30_105")]] * 0.9) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus30_105_95"):=.data[[!!paste0(weather_var_accum,"_totalminus30_105")]] * 0.95) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus30_105_99"):=.data[[!!paste0(weather_var_accum,"_totalminus30_105")]] * 0.99) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus30_105_999"):=.data[[!!paste0(weather_var_accum,"_totalminus30_105")]] * 0.999) %>% 
        
        mutate(!!paste0(weather_var_accum,"_totalminus60_105_90"):=.data[[!!paste0(weather_var_accum,"_totalminus60_105")]] * 0.9) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus60_105_95"):=.data[[!!paste0(weather_var_accum,"_totalminus60_105")]] * 0.95) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus60_105_99"):=.data[[!!paste0(weather_var_accum,"_totalminus60_105")]] * 0.99) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus60_105_999"):=.data[[!!paste0(weather_var_accum,"_totalminus60_105")]] * 0.999) %>% 
        
        mutate(!!paste0(weather_var_accum,"_totalminus1_105_90"):=.data[[!!paste0(weather_var_accum,"_totalminus1_105")]] * 0.9) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus1_105_95"):=.data[[!!paste0(weather_var_accum,"_totalminus1_105")]] * 0.95) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus1_105_99"):=.data[[!!paste0(weather_var_accum,"_totalminus1_105")]] * 0.99) %>%
        mutate(!!paste0(weather_var_accum,"_totalminus1_105_999"):=.data[[!!paste0(weather_var_accum,"_totalminus1_105")]] * 0.999)
    }
    ...
    for (weather_var_instant in instant_var) {
      weather_final <- weather_final %>%
        ...
        mutate(!!paste0(weather_var_instant,"_avgminus30_60"):=(sum(across(num_range(paste0(weather_var_instant,"_"), 30:60))))/31) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus15_60"):=(sum(across(num_range(paste0(weather_var_instant,"_"), 15:60))))/46) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus15_105"):=(sum(across(num_range(paste0(weather_var_instant,"_"), 15:105))))/91) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus30_105"):=(sum(across(num_range(paste0(weather_var_instant,"_"), 30:105))))/76) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus60_105"):=(sum(across(num_range(paste0(weather_var_instant,"_"), 60:105))))/46) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus1_105"):=(sum(across(num_range(paste0(weather_var_instant,"_"), 1:105))))/105) %>%
        ...
        mutate(!!paste0(weather_var_instant,"_avgminus15_105_90"):=.data[[!!paste0(weather_var_instant,"_avgminus15_105")]] * 0.9) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus15_105_95"):=.data[[!!paste0(weather_var_instant,"_avgminus15_105")]] * 0.95) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus15_105_99"):=.data[[!!paste0(weather_var_instant,"_avgminus15_105")]] * 0.99) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus15_105_999"):=.data[[!!paste0(weather_var_instant,"_avgminus15_105")]] * 0.999) %>% 
        
        mutate(!!paste0(weather_var_instant,"_avgminus30_105_90"):=.data[[!!paste0(weather_var_instant,"_avgminus30_105")]] * 0.9) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus30_105_95"):=.data[[!!paste0(weather_var_instant,"_avgminus30_105")]] * 0.95) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus30_105_99"):=.data[[!!paste0(weather_var_instant,"_avgminus30_105")]] * 0.99) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus30_105_999"):=.data[[!!paste0(weather_var_instant,"_avgminus30_105")]] * 0.999) %>% 
        
        mutate(!!paste0(weather_var_instant,"_avgminus60_105_90"):=.data[[!!paste0(weather_var_instant,"_avgminus60_105")]] * 0.9) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus60_105_95"):=.data[[!!paste0(weather_var_instant,"_avgminus60_105")]] * 0.95) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus60_105_99"):=.data[[!!paste0(weather_var_instant,"_avgminus60_105")]] * 0.99) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus60_105_999"):=.data[[!!paste0(weather_var_instant,"_avgminus60_105")]] * 0.999) %>% 
        
        mutate(!!paste0(weather_var_instant,"_avgminus1_105_90"):=.data[[!!paste0(weather_var_instant,"_avgminus1_105")]] * 0.9) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus1_105_95"):=.data[[!!paste0(weather_var_instant,"_avgminus1_105")]] * 0.95) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus1_105_99"):=.data[[!!paste0(weather_var_instant,"_avgminus1_105")]] * 0.99) %>%
        mutate(!!paste0(weather_var_instant,"_avgminus1_105_999"):=.data[[!!paste0(weather_var_instant,"_avgminus1_105")]] * 0.999)
    }
  ...
  pattern <- "^(?:hv045[abc]|hv046|hv80[1-4]|hv807(?:d|m|y|c|a)|hv117_\\d{2}|hv120_\\d{2}|awfact(?:t|u|r|e|w)|m\\d+(?:[a-z]+)?|s\\d+(?:[a-z]+)?|ml\\d+(?:[a-z]+)?)$"
  
  # Remove columns with names matching the pattern
  full <- full %>% 
  ...
  saveRDS(full, file= paste0("~/data-mdavis65/steven_sola/3_Survey_Weather/Rdata/", name_year, "_hhweather.rds"))
  #saveRDS(full, file= paste0("~/scr4-mdavis65/ssola1/Survey_Weather/Rdata/", name_year, "_hhweather.rds"))
...
# Assigning Animal Names --------------------------------------------------------

# Get distinct values in the codebook
codebook <- readRDS("~/data-mdavis65/steven_sola/0_Scripts/ClimateWASH/Aim 3/codebook.rds")
...
if(name_year %in% codebook_distinct) {
  ...
  data <- readRDS(paste0("~/data-mdavis65/steven_sola/3_Survey_Weather/Rdata/",name_year,"_hhweather.rds"))
  ...
  codebook <- codebook %>% 
    group_by(name_year, HV246_animal_recode) %>% 
    mutate(HV246_animal_recode_unique = paste0(HV246_animal_recode, "_", row_number())) %>%
    ungroup()
  ...
  rename_mapping <- setNames(codebook_subset$HV246_animal_recode_unique, tolower(codebook_subset$code))
  ...
  lala <- data %>% select(name_year, starts_with("hv246"))
  
  data <- saveRDS(data, file=paste0("~/data-mdavis65/steven_sola/3_Survey_Weather/Rdata/",name_year,"_hhweather.rds"))
...
hh_files <- list.files(pattern = "*.rds$", full.names = T)
